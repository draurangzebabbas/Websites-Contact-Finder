...
async function callContactInfoScraper(domain, apiKey, specificPath = '') {
  ...
  try {
    ...
    console.log(`📊 Raw Apify result for ${url}:`, {
      ...
      hasData: !!result.emails || !!result.phones
    });
...
async function callContactInfoScraperWithAggregation(domain, apiKey) {
  ...
  try {
    ...
    // Check if we found emails on main page
    if (aggregatedData.emails.length > 0) {
      ...
      return aggregatedData;  // ✅ STOP HERE if emails found!
    } else {
      console.log(`📧 No emails found on main page for ${domain}, continuing to next page`);
    }
  } catch (error) {
    console.error(`❌ Error scraping main page for ${domain}:`, error.message);
    console.log(`⚠️ Continuing to next page despite main page error`);
  }
  ...
  try {
    ...
    // Check if we found emails on /contact page
    if (aggregatedData.emails.length > 0) {
      console.log(`📧 Emails found on /contact page for ${domain}, stopping here`);
      aggregatedData.page_scraped = `https://${domain} (main + /contact)`;
      return aggregatedData;  // ✅ STOP HERE if emails found!
    } else {
      console.log(`📧 No emails found on /contact page for ${domain}, continuing to next page`);
    }
  } catch (error) {
    console.error(`❌ Error scraping /contact page for ${domain}:`, error.message);
    console.log(`⚠️ Continuing to next page despite /contact page error`);
  }
  ...
  try {
    ...
    console.log(`⚠️ All pages checked for ${domain}`);
  }
